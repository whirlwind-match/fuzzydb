...
public interface Store extends Transaction {
	...
	public Collection<Class<?>> getDbClasses(); // returns a list of all classes stored in the DB
	public Class<?> getDbClass(String className); // if you know the string but not the class, this lets you get the class (bytecode download) - this represents a new feature I hadn't considered. It's obviously up to the app to store it or reflectively access its members etc. This is easy enough to do, and actually is a good idea becuase the class upload thing was indroducing asymetry into the new coms design which I didn't like, this actually smplifies things a bit.
	public Collection<String> getNamespaces(Class<?> dbClass); // Gets all namespaces in which the specified class appears.
